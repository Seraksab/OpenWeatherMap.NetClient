...
internal interface IForecast5DaysApiClient
{
  ...
  Task<ApiForecast5DaysResponse> Forecast(string appid, string lang, double lat, double lon, int cnt);
  ...
  /// <param name="cnt">Number of timestamps which will be returned</param>
  [Get("/data/2.5/forecast")]
  Task<ApiForecast5DaysResponse> Forecast(string appid, string lang, int id, int cnt);
}